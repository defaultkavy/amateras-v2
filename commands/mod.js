                        ...
                        case "close":
                            ...
                            if (!value3.user)
                                ...
                                if (player === 404)
                                    ...
                                    _guild.log.send(`${yield _guild.log.name(moderator.id)} 关闭了 ${yield _guild.log.name(player.id)} 的房间`, 'MOD');
                                }
                            }
                            break;
                    }
                        ...
                        case 'default_role':
                            ...
                            if (role) {
                                ...
                                if (_role === 404)
                                    return interaction.reply({ content: `Error: Role fetch failed`, ephemeral: true });
                                if (_role.get.tags)
                                    return interaction.reply({ content: `此身份组无法登记`, ephemeral: true });
                                const botPosition = _guild.amateras ? _guild.amateras.roles.highest.position : undefined;
                                if (!botPosition)
                                    return interaction.reply({ content: `Error: Amateras role fetch failed`, ephemeral: true });
                                if (_role.get.position > botPosition)
                                    return interaction.reply({ content: `此身份组权限位置高于天照，天照无法将新成员加入此身份组`, ephemeral: true });
                                if (enable === undefined) {
                                ...
                                if (enable) {
                                    ...
                                    if (set === 101)
                                        return interaction.reply({ content: `${_role.mention()} 默认身分组设定保持为：${enable ? '开' : '关'}`, ephemeral: true });
                                    else if (set === 404)
                                        return interaction.reply({ content: `Error: Setting failed`, ephemeral: true });
                                }
                                else if (enable === false) {
                        ...
                        case 'welcome_channel':
                            ...
                            if (subcmd1.options) {
                                ...
                                if (enable) {
                                    ...
                                    if (set === 101)
                                        return interaction.reply({ content: `${_channel.get} 欢迎频道设定保持为：${enable ? '开' : '关'}`, ephemeral: true });
                                    else if (set === 404)
                                        return interaction.reply({ content: `Error: Setting failed`, ephemeral: true });
                                }
                                else if (enable === false) {
                                    ...
                                    if (set === 101)
                                        return interaction.reply({ content: `${_channel.mention()} 欢迎频道设定保持为：${enable ? '开' : '关'}`, ephemeral: true });
                                    else if (set === 404)
                                        return interaction.reply({ content: `Error: Setting failed`, ephemeral: true });
                                }
