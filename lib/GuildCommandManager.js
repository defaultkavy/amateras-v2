...
class GuildCommandManager {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (__classPrivateFieldGet(this, _GuildCommandManager_commands, "f")) {
                for (const appCommand of appCommands) {
                    const data = yield __classPrivateFieldGet(this, _GuildCommandManager_collection, "f").findOne({ id: appCommand[0], guild: __classPrivateFieldGet(this, _GuildCommandManager__guild, "f").id });
    ...
    edited() {
        return __awaiter(this, void 0, void 0, function* () {
            const collection = __classPrivateFieldGet(this, _GuildCommandManager_amateras, "f").db.collection('sys');
            const data = yield collection.findOne({ name: 'guild_commands' });
            // Check if database no record OR record different with commandsList
            if (!data || (data && JSON.stringify(data.commands) !== JSON.stringify(__classPrivateFieldGet(this, _GuildCommandManager_commandsList, "f")))) {
                // [] reset guild list
                yield this.record([]);
                return true;
            }
            if (data.guilds instanceof Array && data.guilds.includes(__classPrivateFieldGet(this, _GuildCommandManager__guild, "f").id)) {
                return false;
            }
            else {
                // Pass guild[] from record
                yield this.record(data.guilds, data);
                return true;
            }
        });
    }
    record(guilds, data) {
        return __awaiter(this, void 0, void 0, function* () {
            const collection = __classPrivateFieldGet(this, _GuildCommandManager_amateras, "f").db.collection('sys');
            guilds.push(__classPrivateFieldGet(this, _GuildCommandManager__guild, "f").id);
            const newData = {
                name: 'guild_commands',
                commands: __classPrivateFieldGet(this, _GuildCommandManager_commandsList, "f"),
                guilds: guilds
            };
            if (data) {
                yield collection.replaceOne({ name: 'guild_commands' }, newData);
            }
            else {
    ...
    removeRecord() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (data) {
                if (data.guilds instanceof Array && data.guilds.includes(__classPrivateFieldGet(this, _GuildCommandManager__guild, "f").id)) {
                    data.guilds = (0, terminal_1.removeArrayItem)(data.guilds, __classPrivateFieldGet(this, _GuildCommandManager__guild, "f").id);
                    yield collection.replaceOne({ name: 'guild_commands' }, data);
                }
                else
                    return;
            }
